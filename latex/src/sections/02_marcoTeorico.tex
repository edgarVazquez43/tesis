...
Los principales parámetros que caracterizan a los robots industriales son:
	...
	\begin{itemize}
		\item{Número de grados de libertad. Puede entenderse como número total de variables necesarias para obtener la ubicación de un cuerpo en el espacio. Aunque la mayoría de las aplicaciones requieren manipuladores de 6 DOF, otras más complejas requieren un número mayor, tal es el caso de las labores de apertura y cierre de puertas.}
	...
	\end{itemize}
		...
		\subsection{Cinemática directa}
			...
			Este tema se abordará con más detalle 4.3 en el capítulo de este documento.\\
...
%%  ----------------------------------------------
	\section{Imágenes RGB-D}
		...
		Por lo tanto, necesitamos considerar una fuente de datos adicional, en este caso la profundidad, para discriminar objetos que no están en el rango de interés. En este documento se reportan los datos obtenidos al resolver este problema con la incorporación de un sensor RGB-D. Una imagen RGB-D es una imagen que además de proporcionar indormación del color de la escena agrega información sobre la distancia en profundidad a la cual se encuentran los puntos de la escena.\\
	...
	\section{Algoritmo RANSAC}
		...
		\textbf{Umbral de distancia $t$ para determinar compatibilidad dato/modelo.} Se quiere elegir un umbral $t$ tal que la probabilidad de que un punto pertence es un valor típico es $\alpha$. Este cálculo requiere conocer la distribución de probabilidad de la distancia de un valor típico del modelo.\\

		\textbf{Número de iteraciones.} El número de iteraciones $N$ se calcula en función de la probabilidad $p$ de que un por lo  menos  un  subconjunto  que  se  elige  aleatoriamente  esta  libre  de valores atípicos.\\

		\textbf{Número mínimo $T$ de puntos en el subconjunto de decisión.} El número T se busca que sea similar al número de inliers que se supone que hay en el conjunto de datos.\\
	...
	\section{Algoritmo PCA}
		...
		El algoritmo PCA tiene sus bases en la estadística y parte de la idea de que se tiene un gran conjunto de datos, y se desea analizar ese conjunto en términos de las relaciones entre los puntos individuales que lo conforman. Se pretende analizar algunas de las medidas que podemos calcular en un conjunto de datos y lo que estas medidas dicen acerca de los datos en sí.\\
